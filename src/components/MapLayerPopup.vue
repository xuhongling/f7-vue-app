  ...
  <f7-sheet class="mapLayerPopup" :opened="sheetOpened" @sheet:closed="sheetOpened = false" swipe-to-close swipe-to-step backdrop >
    ...
    <div class="sheet-modal-swipe-step">
      ...
      <ul class="sheetStepList" v-if="this.monitorObj.typeID == 'PP'">
        <li><span>位置：</span><p>{{ monitorObj.RGNNM }}</p></li>
        <li><span>当前雨量：</span><p>{{ monitorObj.S === null ? "" : monitorObj.S }}</p></li>
      ...
      <ul class="sheetStepList" v-if="this.monitorObj.typeID == 'ZZ'">
        ...
        <li><span>当前水位：</span><p>{{ monitorObj.Z === null ? "" : monitorObj.Z }}</p></li>
        <li><span>获取时间：</span><p>{{ monitorObj.TM }}</p></li>
      </ul>
      <!--泵站 -->
      <ul class="sheetStepList" v-if="this.monitorObj.typeID == 'DP'">
        <li><span>位置：</span><p>{{ monitorObj.stlc }}</p></li>
        <li><span>当前站上水位：</span><p></p><p>{{ monitorObj.PPUPZ === null ? "" : monitorObj.PPUPZ }}</p></li>
        <li><span>当前站下水位：</span><p></p><p>{{ monitorObj.PPDWZ === null ? "" : monitorObj.PPDWZ }}</p></li>
        <li><span>获取时间：</span><p>{{ monitorObj.TM }}</p></li>
      </ul>
      <!--水库-->
      <ul class="sheetStepList" v-if="this.monitorObj.typeID == 'RR'">
        <li><span>位置：</span><p>{{ monitorObj.stlc }}</p></li>
        <li><span>当前水位：</span><p></p><p>{{ monitorObj.RZ === null ? "" : monitorObj.RZ }}</p></li>
        <li><span>获取时间：</span><p>{{ monitorObj.TM }}</p></li>
      </ul>
      <!--闸站-->
